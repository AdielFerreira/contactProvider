...
class NotesAdapter(private val listenner: NoteClickListenner):RecyclerView.Adapter<NotesViewHolder>() {
    ...
    override fun onBindViewHolder(holder: NotesViewHolder, position: Int) {
        ...
        holder.noteButtonRemove.setOnClickListener{
            mCursor?.moveToPosition(position)
            listenner.noteRemoveItem(mCursor)
            notifyDataSetChanged()
        }

        holder.itemView.setOnClickListener{listenner.noteClickedItem(mCursor as Cursor)}
    }
